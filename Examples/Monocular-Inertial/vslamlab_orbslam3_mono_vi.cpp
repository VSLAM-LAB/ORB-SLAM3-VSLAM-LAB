...
int main(int argc, char *argv[])
{
    ...
    for (int i = 0; i < argc; ++i) {
        ...
        if (arg.find("calibration_yaml:") != std::string::npos) {
            ...
            std::cout << "[vslamlab_orbslam3_mono_vi.cpp] Path to calibration.yaml = " << calibration_yaml << std::endl;
            continue;
        }
        ...
        if (arg.find("vocabulary:") != std::string::npos) {
            ...
            vocabulary = arg;
            std::cout << "[vslamlab_orbslam3_mono_vi.cpp] Path to vocabulary = " << vocabulary << std::endl;
            continue;
        }
    }
    ...
    LoadImages(sequence_path, rgb_csv, imageFilenames, timestampsCam, cam_name);
